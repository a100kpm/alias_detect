function [TABLE_FLAG,Player_stats,list_match] = MyLittleGuardianV3(datamatch,Player_stats,Hero,list_match)
% function [TABLE_FLAG,Player_stats,list_match] = MyLittleGuardianV3(newMatch_id,Player_stats,Hero,list_match)
%% #1 faire une fonction qui sort les infos du joueur en l'appelant juste par le nom du joueur (ou son id)
%% #2 faire une fonction pour rajouter manuellement a postériori les data d'un joueur d'une game ou il a été flag méchamment
%% #3 faire une fonction pour les items actifs (proba qu'un item soit dans un slot "actif" suivant le nombre d'item actif)
%% il faut list_match=[], Player_stats=table la toute première fois qu'il fonctionne.
%% https://fr.mathworks.com/help/stats/pca.html
% newMatch_id = 4695790012
% datamatch=ApiGetStatOpen(newMatch_id);
%%% le ApiGetStatOpen sera fait a l'extérieur de littleguardian à terme
