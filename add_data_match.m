function match_id_list=add_data_match(conn,match_id_list)
% add data related to tracked player that happened to be in match_id_list

match_id_list_processed=[];
...
%     zone a déplacer au bon endroit pour ne garder que les matchqu'on veut
%     zone a déplacer au bon endroit pour ne garder que les matchqu'on veut
    if retrive==1
        if datamatch.game_mode==21
            retrive=0
        end
    end
%     zone a déplacer au bon endroit pour ne garder que les matchqu'on veut
