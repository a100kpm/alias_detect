function update_player_hero_data(conn,data_player_hero_all)

list_all_id=[data_player_hero_all.player_id,data_player_hero_all.hero_id];
selectQ='select player_hero_data.player_id,player_hero_data.hero_id from player_hero_data where';
nbr=length(list_all_id);

for i=1:nbr
    selectQ=[selectQ,' (player_id=',num2str(list_all_id(i,1)),' and hero_id=',num2str(list_all_id(i,2)),') or'];
end
selectQ(end-2:end)=[];
list_id_update=select(conn,selectQ);
list_id_update=table2array(list_id_update);
list_id_update=double((list_id_update));
list_id_add=setdiff(list_all_id,list_id_update,'row');

whereclause={};
data=[];
for i=1:length(list_id_update)
    whereclause{end+1}=['where player_hero_data.player_id=',num2str(list_id_update(i,1)),' and player_hero_data.hero_id=',num2str(list_id_update(i,2))];
    data=[data;data_player_hero_all(data_player_hero_all.player_id==list_id_update(i,1) & data_player_hero_all.hero_id==list_id_update(i,2),:)];
end
whereclause=whereclause';


list_col={'id','player_id','hero_id','nbr','apm','variance_apm','ratio_apm1','ratio_apm2','ratio_apm3','ratio_apm4',...
...
if ~isempty(data)
    update(conn,'player_hero_data',list_col,data,whereclause)
end

data=data_player_hero_all(ismember([data_player_hero_all.player_id,data_player_hero_all.hero_id],list_id_add,'row'),:);
if ~isempty(data)
    sqlwrite(conn,'player_hero_data',data)
end